...
function makeAdd() {
    ...
    if ( name.trim() == "" || name == null || photo.trim()=="" || photo == null || color.trim() == "" || color == null || size.trim() == "" || size == null) {
        alert("Please You can enter wrong information or empty string");
        return;
    };
    if (quantity < 0 || quantity == "") {
        alert('Quantity cannot be negative or empty string');
        return;
    };
    if (price <= 0 || price > 1000000 || price == "") {
        alert('Price cannot be negative and than more one milion or empty string');
        return;
    };
...
// reset 
function reset() {
    // Gán lại id =  0 để khi reset lại sẽ không giữ id của thằng trc
...
function makeSave() {
    ...
    if (product.price <= 100000 || product.price > 1000000 || product.price == "") {
        alert('Price cannot be cannot be less than one hundred thousand or than more one milion and empty string');
        return;
    };
